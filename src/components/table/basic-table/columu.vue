    ...
    renderColumn() {
      ...
      if (column.isUrl) {
        column.render = (h, scope) => {
          return (
            <el-button type="text">
              <router-link to="{ path : `/studentManagement/studentInfo`, query:{ studentId: scope.row.studentUserId, studentName: scope.row.studentName }}">
                {scope.row[scope.column.property]}
              </router-link>
            </el-button>
          );
