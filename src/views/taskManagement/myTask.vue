<template>
  ...
  <div class="my-task-warp">
    ...
    >
      <div slot="header-right">
        <el-button
          ...
          type="primary"
          v-if="Per.handleButtonPer('teach.myWorkbench.myTask.exportTaskData')"
        >
          {{ $t("导出") }}
        </el-button>
      </div>
    ...
    <AssignStudents
      :parentUserId="currentTaskRow.parentUserId"
      :studentIds="[currentTaskRow.studentUserId]"
      v-if="showAssignStudents"
      :show="showAssignStudents"
      :taskIds="[currentTaskRow.id]"
      @close="showAssignStudents = false"
      @refreshPage="reqList(), reqTaskGroup()"
    ></AssignStudents>
  </div>
</template>
<script>
...
import TaskInfoPage from "./components/taskInfo/taskInfoPage.vue";
import CompletedTaskResult from "./components/CompletedTaskResult.vue";
import AssignStudents from "@/views/learnManager/components/assignStudents";
...
export default {
  ...
  setup(props, { root, emit }) {
      ...
      columns: computed(() => {
        ...
        return [
          ...
          },
          {
            ...
            render: (_, { row }) => {
              return (
                <span class={"text-wrap"}>{row.courseProgress || "-"}</span>
              );
            },
          },
          {
            ...
            prop: "assignee",
            render: (_, { row }) => {
              return <span>{row.assignee?.username || "-"}</span>;
            },
          },
          {
            label: root.$t("完成方式"),
    ...
    const reqList = async () => {
      ...
      try {
        const res = await root.$Api.taskManagement.reqMyTaskList(screenData);
        res.list.forEach((e) => {
    ...
    const reqTaskGroup = async () => {
      ...
      try {
        const res = await root.$Api.taskManagement.reqTaskTypeGroup(screenData);
    ...
    const loginUserId = JSON.parse(getStorage("userInfo")).id;
    const showDrawer = (row) => {
      isTaskOwner.value=row.assignee.userId===loginUserId
      ...
      if (row.taskTypeId == 2 /* 分配学生 */) {
        if (!isTaskOwner.value) return;
        showAssignStudents.value = true;
      } else {
        drawer.value = true;
      }
    };
    ...
    /* 分配学生 */
    const showAssignStudents = ref(false);
    ...
    const handelExportExcel = async () => {
      ...
      try {
        ...
        await root.$Api.taskManagement.reqDownloadTaskDate({
          userIdStr,
          postIdStr,
          statusProcess,
          studentUserId,
