  ...
  setup(props, { root }) {
    ...
    const tableState = reactive({
      ...
      columns: computed(() => [
        ...
        { label: root.$t("课时变动"), prop: "amountChange" },
        {
          prop: "type",
          render: (h, { row }) => (
        ...
        { label: root.$t("变动课时归属"), prop: "ownerName" },
        {
          label: root.$t("原因"),
          render: (h, { row }) => {
            return (
    ...
    const queryTableData = async () => {
      try {
        ...
        const data =
          await root.$Api.studentManagement.getParentAccountFlowDetailsList({
            pageNum: tableState.page,
            pageSize: tableState.limit,
            parentUserId: props.parentUserId,
            productId: props.coursePackageId,
