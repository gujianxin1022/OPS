    ...
    >
      ...
      <div v-else class="transfer-info-warp">
        ...
        <div class="mb10 content">
          ...
          <el-button @click="showTransferMethodDialog = true" type="text">{{
            $t("去选择")
          }}</el-button>
        </div>
        <div class="mb10 content">
...
export default {
  ...
  setup(props, { emit, root, refs }) {
    ...
    const openDialog = async () => {
      if (!props.rowData.payChannel || !props.rowData.payChannel) {
        dialogVisible.value = true;
        return;
      }
      const { payRoutingRuleId, payMethodVoList } =
        await root.$Api.financeManagement.reqPayRuleAndPayMethod({
          orderId: props.rowData.orderId,
          payVersion: "1",
        });
      let params = {
        orderId: props.rowData.orderId,
        payMethodId: payMethodVoList[0].id,
        ruleId: payRoutingRuleId,
      };
      transferInfo.value = [];
      const { bankTransferDetail, payChannel } =
        await root.$Api.financeManagement.reqAccountCode(params);
      dialogVisible.value = true;
      isZelle.value = payChannel === "Zelle";
      transferInfo.value = buildInfoList(bankTransferDetail);
    };
    const handleClose = () => {
    ...
    const orderInfo = {
      id: props.rowData.orderId,
      currency: props.rowData.currency,
    };
    onMounted(() => {
      if (props.from === "transferMethod") {
