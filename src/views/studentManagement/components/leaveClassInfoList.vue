<template>
  <div class="leave-class-info-list-container">
    <el-table
      ...
      v-loading="isLoading"
    >
      <el-table-column fixed align="center" width="50" :label="$t('序号')">
      ...
      >
        <template slot-scope="{ row }">
          {{ DateTimeUtils.dateClockTime(row.leaveTime) || "-" }}
        </template>
      </el-table-column>
      <el-table-column
    ...
    const url = computed(() => {
      switch (props.activeType) {
        case LEAVE_CLASS_TYPE.NO_USE:
          return "getNoUseLeaveClassInfoList";
        case LEAVE_CLASS_TYPE.OCCUPANCY:
          return "getOccupyLeaveClassInfoList";
        case LEAVE_CLASS_TYPE.USED:
          return "getUsedLeaveClassInfoList";
        case LEAVE_CLASS_TYPE.FAILURE:
          return "getExpiredLeaveClassInfoList";
      }
    });
    const key = ref(0);
    watch(
