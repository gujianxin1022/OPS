<template>
  ...
  >
    <span slot="title"
      ...
      >
        <div class="tip-wrap">
          <p>1.{{ $t("adjustmentCourse1") }}</p>
          <p>2.{{ $t("调课机会发放时间：") }}</p>
          <p class="sub-info">(1){{ $t("adjustmentCourse2.1") }}</p>
          <p class="sub-info">(2){{ $t("adjustmentCourse2.2") }}</p>
          <p>3.{{ $t("adjustmentCourse3") }}</p>
          <p>4.{{ $t("adjustmentCourse4") }}</p>
          <p>5.{{ $t("请假机会状态描述") }}：</p>
          <p class="sub-info">(1){{ $t("adjustmentCourse5.1") }}</p>
          <p class="sub-info">(2){{ $t("adjustmentCourse5.2") }}</p>
          <p class="sub-info">(3){{ $t("adjustmentCourse5.3") }}</p>
          <p class="sub-info">(4){{ $t("adjustmentCourse5.4") }}</p>
        </div>
      ...
      <el-button
        v-if="
          Per.handleButtonPer(
            'teach.studentMT.studentInfo.sisfStudentFiles.addAdjustmentClassTimes'
          )
        "
        type="primary"
        round
...
import AdjustmentCourseInfoList from "./adjustmentCourseInfoList.vue";
import {
  SUBJECT_TYPE,
  FORMAL_COURSE,
  AUDITION_COURSE,
...
export default defineComponent({
  ...
  components: { AdjustmentCourseInfoList },
  setup(props, { root, emit, refs }) {
    ...
    const reqAdjustmentCourseTimes = async () => {
      leaveClassInfo.value =
        await root.$Api.studentManagement.reqAdjustmentCourseTimes(
          props.studentId,
          props.courseType
        );
    };
    ...
    const addTimes = async () => {
      ...
      await root.$Api.studentManagement.reqManuallyIssueTheNumberOfAdjustmentCourseRequests(
        {
          grantNum: 1,
      ...
      reqAdjustmentCourseTimes();
      refs.myList.getData();
    };

    onMounted(reqAdjustmentCourseTimes);
