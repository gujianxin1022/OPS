<template>
  <div class="group-student-normal-wrap">
    <div class="search-wrap mt10">
      ...
      <el-select filterable style="width:17%" clearable v-model="screenData.name" :placeholder="$t('请选择申请人')">
    ...
    <div class="table-wrap">
      ...
      >
        ...
        <el-table-column align="center" :label="$t('家长ID')" prop="name" width="120">
        ...
        <!-- <el-table-column align="center" :label="$t('my-home-Parent')" prop="name" width="150">
          <template slot-scope="scope">
            <span>
              <router-link :to="{ path : `/studentManagement/parentInfo`, query:{ parentUserId:scope.row.parentUserId }}">
                <el-button type="text">{{ scope.row.parentRealName }}</el-button>
              </router-link>
            </span>
          </template>
        </el-table-column> -->
        <el-table-column align="center" :label="$t('报名班型')" prop="name" width="300">
          <template slot-scope="scope">
            ...
            <!-- <el-button type="text">
              <span>{{ scope.row.classTypeNameZh }}</span>
              <el-tag v-if="scope.row.languageLabel != null" type="success">{{ scope.row.languageLabel | languageLabelFilter }}</el-tag>
              <el-tag v-if="scope.row.ageLabel != null" type="danger">{{ scope.row.ageLabel | ageLabelFilter }}</el-tag>
            </el-button> -->
          </template>
        </el-table-column>
        ...
        <!-- <el-table-column align="center" label="是否付费" prop="name" width="120">
          <template slot-scope="scope">
            <span>{{ scope.row.pay == 0?'未付费':scope.row.pay==1?'付费':'' }}</span>
          </template>
        </el-table-column> -->
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.$Api.groupManagement.getGroupStudentsList(2, pageNum, pageSize, params)
      .then(({ status, data }) => {
        if( status == 200 && data.code == 200 ) {
