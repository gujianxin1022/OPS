  ...
  <div class="my-work-order">
    ...
    >
      ...
      <screen-item :label="$t('工单ID')" label-width="120">
        <el-input
          clearable
          v-model.trim="screenData.thirdTicketId"
      ...
      <screen-item :label="$t('工单预警')" label-width="120">
        <el-select
          v-model="screenData.alertPriority"
          filterable
          clearable
          :collapse-tags="true"
          :placeholder="$t('请选择')"
        >
          <el-option
            v-for="(item, index) in warningOptionList"
            :key="index"
            :label="item.label"
            :value="item.value"
          >
    ...
    >
      <div slot="header-right">
        <AddWorkOrder @refreshList="(tableState.page = 1), reqList()" />
      </div>
      <BasicTable
