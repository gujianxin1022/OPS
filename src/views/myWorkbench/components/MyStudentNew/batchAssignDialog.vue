<template>
  ...
  >
    <div style="margin-bottom: 20px">
      <label>{{ $t('是否企微继承好友或群') }}:</label>
      <el-radio-group v-model="inheritWechat" style="margin-left: 20px">
    ...
    >
      ...
      >
        <el-button type="text"><a
            href="https://priv.cdn.lingoace.com/OPS/批量分配学管导入模板.xlsx"
          >{{
                $t("下载模板")
              }}</a></el-button>
...
export default defineComponent({
  ...
  setup (props, { root, emit }) {
    ...
    const inheritWechat = ref(1);
    const uploadUrl = `${process.env.VUE_APP_BASE_API}/opsapi/studentmanager/api/v1/student/manage/distribution/xg/parent/import`;
    const closeDialog = () => {
    ...
    const handleFile = (url, formData) => {
      axios({
        ...
        try {
          ...
          if (res.code === 200) {
            root.$confirm(
              root.$t('数据上传成功，系统处理完成后将邮件通知您。超过2h未收到邮件请联系相关人员'),
              root.$t("数据导入成功") + '!',
              {
          ...
          downloadElement.download = `数据上传失败.xlsx`; //下载后文件名
    ...
    const upload = ({ file }) => {
      ...
      formData.append("inheritWechat", inheritWechat.value);
