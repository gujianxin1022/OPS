  ...
  <div class="unitReportWrapper">
    <screen-wrapper @search="search" :screenTitle="$t('filter queries')">
      <screen-item
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfFilterByTime'
          )
        "
        :label="$t('Date')"
        :part="2"
      ...
      >
        <CommonTree
          ...
          :hasLAPermission="Per.handleButtonPer('teach.reportMT.firstClassFeedback.filterByLA')"
          :hasCCPermission="Per.handleButtonPer('teach.reportMT.firstClassFeedback.filterByCC')"
          :hasTeachTubePermission="Per.handleButtonPer('teach.reportMT.firstClassFeedback.filterByTeachTube')"
        />
      </screen-item>
      <screen-item
        v-if="Per.handleButtonPer('teach.reportMT.firstClassFeedback.flfFilterByEdition')"
        style="margin-left: -20px"
        :label="$t('版本')"
        :part="4"
        :label-width="labelWidth"
      >
        <el-select filterable clearable v-model="screenData.courseEditionId" :placeholder="$t('请选择')">
          <el-option
      ...
      <screen-item 
        v-if="Per.handleButtonPer('teach.reportMT.firstClassFeedback.flfFilterByLevel')"
        :label="$t('级别')"
        :part="4"
        :label-width="labelWidth"
      >
        <el-select clearable v-model="screenData.courseLevel" :placeholder="$t('请选择')">
          <el-option
      ...
      <screen-item
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfFilterByIfSendToParent'
          )
        "
        :label="$t('海报与短链记录')"
        :part="4"
        :label-width="currentLang === 'en' ? 130 : 110"
      >
        <el-select clearable v-model="screenData.templateDesc" :placeholder="$t('请选择')">
          <el-option
            v-for="item in sendParentList"
            :key="item.firstReportId"
            :label="item.lable"
            :value="item.value"
          />
        </el-select>
      </screen-item>
      <screen-item :label="$t('my-home-Student')" :part="4" :label-width="labelWidth">
        <el-input
          clearable
      ...
      <screen-item
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfFilterByTeacher'
          )
        "
        :label="$t('教师')"
        :part="4"
      ...
      >
        <el-select clearable filterable v-model="screenData.readStatus" :placeholder="请输入">
          <el-option
    ...
    <el-button-group v-if="tableType == 1" class="type-btn">
      <el-button
        ...
        v-if="Per.handleButtonPer('teach.reportMT.firstClassFeedback.flfAllStatus')"
      >{{ $t('全部') }}</el-button>
      <el-button
        size="medium"
        :type="type == 1 ? 'primary' : ''"
        ...
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfWaitingSubmit'
          )
        "
      >{{ $t("to be submitted") }}</el-button>
      <el-button
        size="medium"
        :type="type == 2 ? 'primary' : ''"
        @click="changeType(2)"
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfWaitingReview'
          )
        "
      >{{ $t("待审核") }}</el-button>
      <el-button
        ...
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfReviewFailed'
          )
        "
      >{{ $t("审核未通过") }}</el-button>
      <el-button
        ...
        v-if="
          Per.handleButtonPer(
            'teach.reportMT.firstClassFeedback.flfReviewSuccess'
          )
        "
      >{{ $t("审核通过") }}</el-button>
    </el-button-group>
    <custom-card :title="$t('数据列表')" class="table-wrapper">
      ...
      >
        ...
        <el-table-column fixed :label="$t('授课时间')" prop="startTime" width="180" align="center">
          <template slot-scope="scope">{{ DateTimeUtils.dateClockTime(scope.row.startTime) }}</template>
        </el-table-column>
        <el-table-column
          fixed
          align="center"
        ...
        <el-table-column
          ...
          align="center"
        ></el-table-column>
        <el-table-column :label="$t('my-home-Curriculum')" prop="add" width="190" align="center">
          <template slot-scope="scope">
            <span>
              {{ scope.row.courseEditionId | mapEdtion }} Level{{
                scope.row.courseLevel
              }}
              Lesson{{ scope.row.lessonNo }}
            </span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              "
              type="text"
            >{{ scope.row.studentRealName }}</el-button>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('家长id')"
        ...
        <el-table-column :label="$t('my-home-Age')" prop="studentAge" width="50" align="center"></el-table-column>
        <el-table-column
          :label="$t('课程顾问')"
        ...
        <el-table-column fixed="right" :label="$t('操作')" width="150" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="
                scope.row.status === ApiConstants.reportStNoPass &&
                Per.handleButtonPer(
                  'teach.reportMT.firstClassFeedback.flfViewFLF'
                )
              "
              type="text"
            ...
            >{{ $t("Audit") }}</el-button>
            <!-- 先暂时隐藏罚金按钮 -->
            <!-- <el-button type="text" @click="deductMoneyEvent(scope.row)">
              {{
                $t("Fine")
              }}
            </el-button> -->
            <el-button
              v-if="
                scope.row.status === ApiConstants.reportStPass &&
                Per.handleButtonPer(
                  'teach.reportMT.firstClassFeedback.flfWithdraw'
                )
              "
              type="text"
      ...
      >
        ...
        <el-table-column
          ...
          :label="$t('my-home-Teacher\'s Nickname')"
          prop="tutorRealName"
          :min-width="currentLang === 'en' ? 140 : 120"
          align="center"
        ></el-table-column>
        <el-table-column :label="$t('my-home-Curriculum')" prop="add" width="190" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column fixed="right" :label="$t('操作')" width="150" align="center">
          <!-- <template slot-scope="scope">
            <el-button
              type="text"
              v-if="
                Per.handleButtonPer('teach.reportMT.firstClassFeedback.flfFine')
              "
      ...
      >
        ...
        <el-table-column
          ...
          align="center"
        ></el-table-column>
        <el-table-column :label="$t('my-home-Approval Status')" width="80" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.status | mapUnitReportStatus }}</span>
          </template>
        </el-table-column>
    ...
    >
      <el-form>
          ...
          <div style="display: flex" class="mt10" v-if="appealStopTime">
            ...
            <div style="width: 400px">
              <el-input
                ...
                maxlength="255"
                show-word-limit
                :placeholder="$t('glb-input')+(currentLang=='en'?' ':'')+$t('罚金原因备注信息')"
              />
            </div>
          </div>
        ...
        <el-form-item class="btn-wrapper text-center">
          <el-button v-if="appealStopTime" @click="deductMoneyFlag = false">
            {{
              $t("取消")
            }}
          </el-button>
  ...
  methods: {
    ...
    getListAJAX() {
      ...
      for (var key in this.screenData) {
        ...
        if (key === 'userIdList' || key === 'postIdList') {
          let newKey = key === 'userIdList' ? 'userIdStr' : 'postIdStr';
          this.screenData[key].length
            ? params[newKey] = this.screenData[key].join()
            : null;
          delete params[key];
        }
      }
