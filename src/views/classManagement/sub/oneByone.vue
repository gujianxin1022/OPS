  ...
  <div class="onebyone-wrap">
    ...
    <div class="table-wrap">
      ...
      >
            ...
            <el-button type="text">
              <router-link
                :to="{
                  ...
                  query: {
                    studentId: scope.row.studentUserId,
                    studentName: scope.row.studentName,
                  },
                }"
                >{{ scope.row.studentName }}</router-link
              >
            </el-button>
            <template v-if="type == 3">
              <el-tag
                type="success"
                v-if="scope.row.virtualStudentStatus == '3'"
              >
                {{ $t("已到") }}</el-tag
              >
              <el-tag
                type="danger"
                v-if="scope.row.virtualStudentStatus == '1'"
              >
                {{ $t("未到") }}</el-tag
              >
            </template>
        ...
        >
          <template slot-scope="scope">
            <el-tag type="success" v-if="scope.row.isSubstitute">
              {{ $t("是") }}</el-tag
            >
        ...
        >
          <template slot-scope="scope">
            <p :class="setCourseTypeStyle(scope.row.courseType)">
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.equipment">{{ scope.row.equipment }}</span>
            <span v-else>---</span>
          </template>
        </el-table-column>
        <!-- 学生设备 -->
        ...
        >
          <template slot-scope="scope">
            ...
            <!-- &&
                Per.handleButtonPer('teach.classMT.scheduling.classroomForCC') -->
            <CCVideoOrder
              v-if="
              ...
              :row="scope.row"
              :classTypeId="1"
              :virtualClassId="scope.row.id"
              :virtualclassType="scope.row.virtualclassType"
            />
          </template>
        </el-table-column>
        <!-- 操作 -->
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(["setSchedulingPage", "setSchedulingSearchData"]),
    // 设置课程类型颜色
    setCourseTypeStyle(courseType) {
      if (courseType === undefined || courseType === null) return "";
      return courseType === 0 ? "red" : "green";
    },
    ...
    getXgGw(item) {
      ...
      this.$Api.groupManagement
        .getGwXg(item.studentUserId, item.coursePackageId)
        .then(({ data }) => {
    ...
    clickHandlerAbnormal(virtualclassId) {
      ...
      this.$Api.classManagement
        .getExamineInfo(1, virtualclassId)
        .then(({ status, data }) => {
          if (status == 200 && data.code == 200) {
    ...
    clickHandlerComment(virtualclassId, target, obj) {
      ...
      this.teacherInfo.classType = "1对1";
    ...
    clickHandlerMonitor(virtualclassId) {
      virtualclassMonitor(1, virtualclassId).then(({ status, data }) => {
        if (status == 200 && data.code == 200) {
    ...
    getTableData() {
      ...
      this.tableData = [];
      this.$Api.classManagement
        .getSchedulingOneByOneList({ ...this.screenData, ...this.userParams })
        .then(({ status, data }) => {
          if (status == 200 && data.code == 200) {
