  ...
  <div class="onebyone-wrap">
    <screen-wrapper @search="search" :screenTitle="$t('filter queries')">
      ...
      <screen-item :label="$t('版本')" :part="4" :label-width="labelWidth">
        <el-select
          filterable
          v-model="screenData.programmeName"
          :placeholder="$t('请选择')"
        >
          <el-option
      ...
      <screen-item
        ...
        :label-width="labelWidth"
      >
        <el-select v-model="screenData.student" :placeholder="$t('请选择')">
          <el-option
          ...
          <template slot-scope="scope">
            <router-link
              :to="{
                ...
                query: { id: scope.row.classId, type: 3 },
              }"
            >
...
export default {
  ...
  methods: {
      ...
      this.$refs.singleChangeFrom.validate((valid) => {
        if (valid) {
          ...
          this.$Api.classManagement
            .classArrangementCrConMoreBigClass({
              virtualClassIds: ids,
    ...
    clickHandlerSubstitute(virtualclassId, target, obj) {
      ...
      this.currentType = "bigClass";
    },
    // 旁听
    clickHandlerMonitor(virtualclassId) {
      virtualclassMonitor(3, virtualclassId).then(({ status, data }) => {
        if (status == 200 && data.code == 200) {
    ...
    getTableData() {
      ...
      this.tableData = [];
      this.$Api.classManagement
        .getSchedulingBigClassList({ ...this.screenData, ...this.userParams })
        .then(({ status, data }) => {
          if (status == 200 && data.code == 200) {
