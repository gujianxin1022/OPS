      ...
      >
        ...
        <div class="salary">
          ...
          <el-table :data="optionsState.salaryList" border>
            ...
            <el-table-column
              v-for="(value, key, salaryIdx) in configItem.subject ===
              SUBJECT_TYPE.CHINESE
          ...
          <el-table :data="configItem.rewardDetails" border>
            ...
            >
              <template slot-scope="{ row, $index }">
                <span v-if="$index === 0">{{
                  item === REWARD_DETAILS_COLUMN_KEYS.STUDENT_ATTEND
                    ? $t("不生效")
                    : $t("老师缺席")
                }}</span>
                <span
                  v-else-if="item !== REWARD_DETAILS_COLUMN_KEYS.STUDENT_ATTEND"
                  class="reward-details-operation-item"
                >
                  <el-form-item
                    :show-message="false"
                    :rules="{ required: true, trigger: 'blur' }"
    ...
    const formatConfigList = (rawData) => {
      configList.value = rawData.map((item) => {
        ...
        return {
          ...
          salaryUnit /* 课酬单位 */,
          normalRule /* 全职课酬 */: isZhSubjectType
            ? { ...INIT_SALARY_ROW_DATA }
            : { p0: void 0 },
          trialRule /* 兼职课酬 */: isZhSubjectType
            ? { ...INIT_SALARY_ROW_DATA }
            : { p0: void 0 },
