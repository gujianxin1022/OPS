      ...
      columns() {
        ...
        return [
            ...
            },
            {
                ...
                prop: "checkResult",
                render: (h, { row }) => {
                    return row.checkResult === 0 ? this.$t("合格") : this.$t("不合格");
                },
            },
            {
                label: this.$t("原因"),
                prop: "unqualifyReason",
                render: (h, { row }) => {
                    const reason = row.unqualifyReason?.split(',') || [];
                    return reason.map(item => this.reasonList.find(ite => ite.value === item)?.label).join(',');
                },
            },
            {
                label: this.$t("备注"),
    ...
    methods: {
      search() {
        ...
        this.$Api.tutorManagement
          .getTutorQualityCheckRecord(params)
          .then(( data ) => {
