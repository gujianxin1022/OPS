  ...
  <div class="out-teacher-wrap">
    ...
    <div class="table-wrap">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.birthday | birthdayMapToAge }}</span>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
    ...
    <div>
      ...
      >
        <el-input
          ...
          :rows="7"
          :placeholder="
            $t('glb-input') + currentLang == 'en' ? '...' : '详细原因'
          "
          v-model="remark"
        >
        </el-input>
  ...
  methods: {
    sure() {
      if (this.remark) {
        ...
        this.$notify({
          message:
            this.$t("glb-input") + currentLang == "en" ? "..." : "详细原因",
          type: "error",
        });
      }
    },
    ...
    getTableData() {
      ...
      this.$Api.tutorManagement
        .getTeacherProcessList(9, pageNum, pageSize, params)
        .then(({ status, data }) => {
          if (status == 200 && data.code == 200) {
