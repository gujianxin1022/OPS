        ...
        <div v-for="(rule, ruleIndex) in ruleList" :key="ruleIndex">
          <div class="rule-tab-content">
            ...
            <el-table :data="rule.tableData" border :style="{ width: !isDialog ? '1000px' : '100%' }"
              ...
              <el-table-column :label="$t('条件值')" min-width="150">
                <template slot-scope="scope">
                  <div class="tab-right">
                    <div v-for="(item, index) in scope.row.countriesChild" :key="index">
                      <el-select v-model="scope.row.countriesChild[index]" :placeholder="$t('请选择')" style="width: 120px">
              ...
              <el-table-column :label="$t('计费金额') + ' (' + currency + '/' + $t('课时') + ')'" width="185">
...
<script>
import {
  qualityTestRuleTypeOptions,
  operatorStrOptions,
  operatorNumOptions,
  logicOptions,
  qualityTestIfOptions,
  feeTypeOptions ,
  monthlyIfOptions,
} from "./filterRule";

export default {
  name: 'QualityTestRule',  
  props: {
    currency: {
      type: String,
      default: 'USD'
  ...
  methods: {
        ...
        for (let i = 0; i < this.ruleList.length; i++) {
          ...
          for (let j = 0; j < rule.tableData.length; j++) {
            ...
            if (row.countries.some(country => !country)) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行条件字段不能为空`));  
              return;
            }
            if (row.operator.some(op => !op)) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行运算规则不能为空`));
              return;
            }
            if (row.countriesChild.some(child => !child)) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行条件值不能为空`));  
              return;
            }
            if (!row.deduce && row.deduce !== 0) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行课酬扣除不能为空`));
              return;
            }
            if (isNaN(Number(row.deduce))) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行课酬扣除必须是数字`));
              return;
            }
            // if (Number(row.deduce) < 0) {
            //   reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行课酬扣除不能为负数`));
            //   return;
            // }
            if (Number(row.deduce) > 99999) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行课酬扣除不能超过99999`));
              return;
            }
            const defaultRewardStr = String(row.deduce || '');
            if (defaultRewardStr.includes('.') && defaultRewardStr.split('.')[1].length > 2) {
              reject(new Error(`质检扣款的第${i + 1}个课酬类型的第${j + 1}行课酬扣除最多支持两位小数`));
              return;
            }
          }
        }
    ...
    getAvailableRuleTypes(currentIndex) {
      ...
      return this.qualityTestRuleTypeOptions.filter(
    ...
    // 新加ruleList整条规则
    addRule() {
      if (this.ruleList.length >= this.qualityTestRuleTypeOptions.length) {
