    ...
    <div class="rule-container-content">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="180px" v-if="status" :disabled="isDetails">
        ...
        <el-form-item :label="$t('计费金额') + '(' + currency + '/' + $t('课时') + ')：'" prop="defaultReward" v-if="ruleForm.defaultRule === 1">
          <el-input v-model="ruleForm.defaultReward" :placeholder="$t('请输入')" style="width: 300px"></el-input>
        </el-form-item>
        <!-- 遍历课酬类型规则结构 start-->
        ...
        this.$refs.ruleForm.validate(valid => {
          ...
          if (this.ruleForm.defaultRule === 0) {
            ...
            if (hasEmptysubCategory) {
              reject(new Error('请选择保护期课酬的课酬类型'));
              return;
            }
            ...
            for (let i = 0; i < this.ruleList.length; i++) {
              ...
              for (let j = 0; j < rule.tableData.length; j++) {
                ...
                if (row.countries.some(country => !country)) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件字段不能为空`));  
                  return;
                }
                // 验证运算规则是否为空
                if (row.operator.some(op => !op)) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行运算规则不能为空`));
                  return;
                }
                // 验证条件值是否为空
                ...
                if (row.countriesChild.some(child => !child)) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件值不能为空`));
                  return;
                }
                ...
                if (rule.subCategory === 5) {
                  for (let k = 0; k < row.countriesChild.length; k++) {
                    ...
                    // 检查是否为有效数字
                    if (isNaN(numValue)) {
                      reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件值必须是数字`));
                      return;
                    }
                    
                    // 检查是否为整数
                    if (!Number.isInteger(numValue)) {
                      reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件值必须是整数`));
                      return;
                    }
                    
                    // 检查是否为非负整数
                    if (numValue < 0) {
                      reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件值不能为负数`));
                      return;
                    }
                    
                    // 检查是否超过最大值
                    if (numValue > 99999) {
                      reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行条件值不能超过99999`));
                      return;
                    }
                  }
                }
                ...
                if (!row.reward && row.reward !== 0) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行金额不能为空`));
                  return;
                }
                if (isNaN(Number(row.reward))) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行金额必须是数字`));
                  return;
                }
                if (Number(row.reward) > 99999) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行金额不能超过99999`));
                  return;
                }
                const defaultRewardStr = String(row.reward || '');
                if (defaultRewardStr.includes('.') && defaultRewardStr.split('.')[1].length > 2) {
                  reject(new Error(`保护期课酬的第${i + 1}个课酬类型的第${j + 1}行金额最多支持两位小数`));
                  return;
                }
              }
            }
