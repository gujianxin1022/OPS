...
import { UnitDetailServices,conversionSchema } from './services/api'
...
export default {
  ...
  methods: {
   ...
   getUnitKnowledgeList(){
        UnitDetailServices.getUnitKnowledgeList({
   ...
   unitKnowledgeSave() {
      this.unitKnowledgeForm.submit().then(async (res)=>{
            ...
            let data = await UnitDetailServices.saveUnitKnowledge({
              unitId: this.$route.query.unitId,
    ...
    async unitKnowledgeSubmit(...v) {
      ...
      let ids = this.existDeleteIds({
       ...
       let data = await UnitDetailServices.updateUnitKnowledge({
        unitId: this.$route.query.unitId,
        courseUnitKnowledgePointsIds:ids,
        courseUnitKnowledgePoints:newParam
      });
    ...
    getUnitSkillList(){
      UnitDetailServices.getUnitSkillList({
          unitId: this.$route.query.unitId,
          status:0
      }).then(res=>{
           if(res.data.code = 200){
   ...
   async unitSkillSave() {
      this.unitSkillForm.submit().then(async (res)=>{
            ...
            let data = await UnitDetailServices.saveUnitSkill({
              unitId: this.$route.query.unitId,
    ...
    async unitSkillSubmit(...v) {
      ...
      let data = await UnitDetailServices.updateUnitSkill({
        unitId:this.$route.query.unitId,
        courseUnitSkills:newParam,
        courseUnitSkillsIds:ids
      });
   ...
   getCommunicationList(){
        UnitDetailServices.getCommunicationList({
          unitId: this.$route.query.unitId,
          status:0
        }).then(res=>{
           if(res.data.code = 200){
   ...
   communicationSave() {
        this.communicationForm.submit().then(async (res)=>{
            ...
            let data = await UnitDetailServices.saveOrUpdateCommunication({
              status:0,
              unitId:this.$route.query.unitId,
